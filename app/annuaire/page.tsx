...
export default function Annuaire() {
  ...
  async function inscrire(e: React.FormEvent) {
    ...
    else { setEnvoi("succes"); setNouveau({ nom: "", prenom: "", promotion: "", filiere: "", secteur: "", ville: "" }); setPhoto(null); setApercu(null); chargerAnciens(); }
  }
    ...
    <>
      <section className="bg-[#1E5A8E] text-white py-16">
        <div className="max-w-[1120px] mx-auto px-5 md:px-8 flex flex-col md:flex-row md:items-end justify-between gap-6">
          ...
          <button onClick={() => { setFormulaire(true); setEnvoi("idle"); }} className="bg-[#B5966E] text-white font-semibold text-sm px-6 py-3 rounded-sm whitespace-nowrap shrink-0 hover:bg-[#8B7355] transition">
        ...
        <div className="max-w-[1120px] mx-auto px-5 md:px-8">
          ...
          {chargement ? (
            ...
            <div className="text-center py-20">
              ...
              <button onClick={() => { setFormulaire(true); setEnvoi("idle"); }} className="bg-[#1E5A8E] text-white text-sm font-medium px-5 py-2.5 rounded-sm">Être le premier à s&apos;inscrire</button>
            </div>
          ) : (
            <>
              <div className="text-[12.5px] text-[#9a9a9a] italic mb-6">{resultats.length} membre{resultats.length > 1 ? "s" : ""} trouvé{resultats.length > 1 ? "s" : ""}</div>
              {resultats.length === 0 ? (
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-px bg-[#D5C9B8] border border-[#D5C9B8]">
                  {resultats.map((a) => (
                    <button key={a.id} onClick={() => setSelection(a)} className="bg-[#FAFAF8] p-6 text-left hover:bg-[#F0EAE0] transition cursor-pointer">
              ...
              <form onSubmit={inscrire} className="flex flex-col gap-4">
                ...
                {[
                  ...
                  <div key={f.key}>
                    ...
                    <input type="text" placeholder={f.placeholder} required value={nouveau[f.key as keyof typeof nouveau]} onChange={(e) => setNouveau({ ...nouveau, [f.key]: e.target.value })} className="w-full border border-[#D5C9B8] bg-white rounded-sm px-4 py-2.5 text-sm focus:outline-none focus:border-[#1E5A8E]" />
                  </div>
                ))}
