...
export default function Header() {
  ...
  useEffect(() => {
    function handleEsc(e: KeyboardEvent) { if (e.key === "Escape") setOuvert(false); }
        ...
        <div className="hidden md:flex items-center gap-3">
          {connecte ? (
            <>
              ...
              <button onClick={deconnecter} className="border border-[#D5C9B8] text-[#6B6B6B] text-sm font-medium px-4 py-2 rounded-sm hover:border-[#1E5A8E] hover:text-[#1E5A8E] transition">
                Déconnexion
              </button>
            </>
          ) : (
            <>
              ...
              <Link href="/inscription" className="bg-[#1E5A8E] text-white text-sm font-medium px-5 py-2.5 rounded-sm hover:bg-[#2970AA] transition">
                Rejoindre
              </Link>
            </>
          )}
