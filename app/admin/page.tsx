...
type OrientationItem = { id: string; type: string; titre: string; texte: string; auteur: string; statut: string; };
...
export default function Admin() {
  ...
  const [orientations, setOrientations] = useState<OrientationItem[]>([]);
  ...
  function Actions({ table, item }: { table: string; item: { id: string; statut: string } }) {
    return (
      <div className="flex gap-2 flex-wrap mt-3">
        {item.statut !== "validé" && <button onClick={() => valider(table, item.id)} className="bg-green-600 text-white text-[12px] px-3 py-1.5 rounded-sm min-h-[36px]">Valider</button>}
        {item.statut !== "rejeté" && <button onClick={() => rejeter(table, item.id)} className="bg-yellow-500 text-white text-[12px] px-3 py-1.5 rounded-sm min-h-[36px]">Rejeter</button>}
        <button onClick={() => supprimer(table, item.id)} className="bg-red-600 text-white text-[12px] px-3 py-1.5 rounded-sm min-h-[36px]">Supprimer</button>
      </div>
    );
  }
  ...
  if (chargementAuth) {
    return (
      <div className="min-h-screen bg-[#1E5A8E] flex items-center justify-center">
        <div className="text-white text-sm">Vérification...</div>
      </div>
    );
  }
          ...
          <form onSubmit={connecter} className="flex flex-col gap-4">
            <div>
              ...
              <input
                ...
                className="w-full border border-[#D5C9B8] rounded-sm px-4 py-2.5 text-sm focus:outline-none focus:border-[#1E5A8E]"
              />
            </div>
            <div>
            ...
            <button type="submit" className="bg-[#1E5A8E] text-white font-medium px-5 py-2.5 rounded-sm min-h-[44px]">
              Accéder
            </button>
          </form>
        </div>
  ...
  return (
    <div className="min-h-screen bg-[#FAFAF8]">
      <div className="bg-[#1E5A8E] text-white px-5 md:px-8 py-4 flex justify-between items-center">
        ...
        <button onClick={deconnecter} className="text-sm text-[#A8CBE8] hover:text-white min-h-[44px] px-3">Déconnexion</button>
      </div>
      ...
      <div className="max-w-[1120px] mx-auto px-5 md:px-8 py-8">
        <div className="flex gap-2 mb-8 border-b border-[#D5C9B8] pb-4 flex-wrap">
          {(["annuaire", "actualites", "offres", "orientation", "bureau", "conseillers"] as const).map((o) => (
            <button key={o} onClick={() => setOnglet(o)} className={"px-4 py-2 rounded-sm text-sm font-medium transition min-h-[44px] " + (onglet === o ? "bg-[#1E5A8E] text-white" : "bg-[#F0EAE0] text-[#6B6B6B]")}>
              {o === "annuaire" ? "Annuaire" : o === "actualites" ? "Actualités" : o === "offres" ? "Offres" : o === "orientation" ? "Orientation" : o === "bureau" ? "Bureau" : "Conseillers"}
            </button>
          ))}
          <button onClick={chargerTout} className="ml-auto px-4 py-2 text-sm text-[#6B6B6B] border border-[#D5C9B8] rounded-sm min-h-[44px]">↻ Actualiser</button>
        </div>
            ...
            {onglet === "bureau" && (
              <div>
                <div className="flex justify-between items-baseline mb-4 flex-wrap gap-2">
                  ...
                  <button onClick={() => { setModalBureau("ajouter"); setFormBureau({ nom: "", role: "", whatsapp: "", mail: "", ordre: bureau.length + 1 }); setEnvoiBureau("idle"); }}
                    className="bg-[#1E5A8E] text-white text-sm font-medium px-4 py-2 rounded-sm min-h-[44px]">
                    + Ajouter
                  </button>
                </div>
                <div className="flex flex-col gap-px bg-[#D5C9B8] border border-[#D5C9B8]">
                  ...
                  {bureau.map((m) => (
                    <div key={m.id} className="bg-[#FAFAF8] p-5">
                      <div className="flex justify-between items-start flex-wrap gap-2">
                        <div>
                          ...
                          <div className="flex gap-2 mt-3">
                            <button onClick={() => { setSelectionBureau(m); setFormBureau({ nom: m.nom, role: m.role, whatsapp: m.whatsapp, mail: m.mail, ordre: m.ordre }); setModalBureau("modifier"); setEnvoiBureau("idle"); }}
                              className="bg-[#1E5A8E] text-white text-[12px] px-3 py-1.5 rounded-sm min-h-[36px]">Modifier</button>
                            <button onClick={() => { setSelectionBureau(m); setModalBureau("supprimer"); setEnvoiBureau("idle"); }}
                              className="bg-red-600 text-white text-[12px] px-3 py-1.5 rounded-sm min-h-[36px]">Supprimer</button>
                          </div>
                        </div>
                      </div>
                    </div>
            ...
            {onglet === "conseillers" && (
              <div>
                <div className="flex justify-between items-baseline mb-4 flex-wrap gap-2">
                  ...
                  <button onClick={() => { setModalConseiller("ajouter"); setFormConseiller({ nom: "", whatsapp: "", mail: "", ordre: conseillers.length + 1 }); setEnvoisConseiller("idle"); }}
                    className="bg-[#1E5A8E] text-white text-sm font-medium px-4 py-2 rounded-sm min-h-[44px]">
                    + Ajouter
                  </button>
                </div>
                <div className="flex flex-col gap-px bg-[#D5C9B8] border border-[#D5C9B8]">
                  ...
                  {conseillers.map((c) => (
                    <div key={c.id} className="bg-[#FAFAF8] p-5">
                      <div className="flex justify-between items-start flex-wrap gap-2">
                        <div>
                          ...
                          <div className="flex gap-2 mt-3">
                            <button onClick={() => { setSelectionConseiller(c); setFormConseiller({ nom: c.nom, whatsapp: c.whatsapp, mail: c.mail, ordre: c.ordre }); setModalConseiller("modifier"); setEnvoisConseiller("idle"); }}
                              className="bg-[#1E5A8E] text-white text-[12px] px-3 py-1.5 rounded-sm min-h-[36px]">Modifier</button>
                            <button onClick={() => { setSelectionConseiller(c); setModalConseiller("supprimer"); setEnvoisConseiller("idle"); }}
                              className="bg-red-600 text-white text-[12px] px-3 py-1.5 rounded-sm min-h-[36px]">Supprimer</button>
                          </div>
                        </div>
                      </div>
            ...
            {envoiBureau === "succes" ? (
              ...
              <form onSubmit={modalBureau === "ajouter" ? ajouterMembre : modifierMembre} className="flex flex-col gap-4">
                ...
                <div>
                  ...
                  <input type="file" accept=".jpg,.jpeg,.png" onChange={(e) => setPhotoBureau(e.target.files?.[0] || null)}
                  ...
                  {!photoBureau && selectionBureau?.photo_url && <p className="text-[12px] text-[#9a9a9a] mt-1">Photo actuelle déjà enregistrée</p>}
                </div>
                ...
                <div className="flex gap-3 mt-2">
                  <button type="submit" disabled={envoiBureau === "chargement"}
                    className="bg-[#1E5A8E] text-white text-sm font-medium px-5 py-2.5 rounded-sm disabled:opacity-60 min-h-[44px]">
      ...
      {modalBureau === "supprimer" && selectionBureau && (
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-30 p-6" onClick={() => setModalBureau("")}>
          <div className="bg-[#FAFAF8] rounded max-w-sm w-full p-8 border border-[#D5C9B8]" onClick={(e) => e.stopPropagation()}>
            ...
            <p className="text-[13.5px] text-[#6B6B6B] mb-6"><strong>{selectionBureau.nom}</strong> ({selectionBureau.role}) sera retiré définitivement.</p>
            ...
            <div className="flex gap-3">
              <button onClick={supprimerMembre} disabled={envoiBureau === "chargement"}
                className="bg-red-600 text-white text-sm font-medium px-5 py-2.5 rounded-sm disabled:opacity-60 min-h-[44px]">
      ...
      {modalConseiller === "supprimer" && selectionConseiller && (
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-30 p-6" onClick={() => setModalConseiller("")}>
          <div className="bg-[#FAFAF8] rounded max-w-sm w-full p-8 border border-[#D5C9B8]" onClick={(e) => e.stopPropagation()}>
            ...
            <p className="text-[13.5px] text-[#6B6B6B] mb-6"><strong>{selectionConseiller.nom}</strong> sera retiré définitivement.</p>
